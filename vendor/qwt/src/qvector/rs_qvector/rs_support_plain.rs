...
impl<const B_SIZE: usize> RSSupportPlain<B_SIZE> {
    ...
    /// Superblock counter lines for E5.10 flatten.
    #[inline]
    pub fn superblocks(&self) -> &[SuperblockPlain] {
        &self.superblocks
    }

    /// Select samples for symbol `s ∈ 0..4` (E5.10 flatten).
    #[inline]
...
///
/// Public for E5.10 flatten / mmap (byte-identical on-disk layout).
...
impl SuperblockPlain {
    ...
    ///
    /// Public for E5.11 B2 `range_counts4` (mapped fused expand loads one superblock once).
    #[inline(always)]
    ...
    /// Superblock-prefix counter for `symbol` (E5.10 mapped select).
    #[inline(always)]
