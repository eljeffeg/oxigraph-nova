...
/// index files.
pub(crate) fn index_dir(data_dir: &Path) -> PathBuf {
...
pub(crate) fn read_marker(data_dir: &Path) -> Option<u64> {
...
pub(crate) fn write_marker(data_dir: &Path, generation: u64) -> Result<(), Oxigraph> {
...
pub(crate) fn index_quad_insert(
    ft: &FulltextIndex,
    dict: &Dictionary,
    g_id: GraphId,
    s_id: TermId,
...
pub(crate) fn index_quad_remove(
    ft: &FulltextIndex,
    g_id: GraphId,
    s_id: TermId,
    p_id: TermId,
...
/// Full rebuild: walk every graph's current triples (post-compaction Ring
/// state) and (re-)index every literal object. Used when the generation
/// marker is missing/stale at `enable_fulltext` time — deliberately does
/// **not** call `ft.commit()`; the caller does that once, after also folding
/// in any quads still sitting in the (not-yet-compacted) delta.
///
/// Clears the index first (buffered — not visible until the caller's
/// `commit()`), so a rebuild triggered against an index that already has
/// *some* documents (e.g. `enable_fulltext` called again after the marker
/// went stale, or a previous rebuild that didn't fully commit) starts from
/// a clean slate instead of duplicating every re-indexed literal alongside
/// its stale copy.
