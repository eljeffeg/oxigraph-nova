...
pub struct StoreDataset<S: QuadStore + ?Sized> {
    store: Arc<S>,
}

impl<S: QuadStore + ?Sized> StoreDataset<S> {
...
impl<S: QuadStore + ?Sized + 'static> DatasetLftjSource for StoreDataset<S> {
...
impl<S: QuadStore + ?Sized + 'static> Dataset for StoreDataset<S> {
    fn find_quads<'a>(&'a self, pattern: &QuadPattern) -> Result<QuadIter<'a>> {
