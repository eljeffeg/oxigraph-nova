...
pub trait PreparedPhysicalOperator: Send {
    ...
    ///
    /// `Result<_, ()>` is intentional: cancellation is a pure control-flow
    /// signal with no diagnostic payload (see clippy::result_unit_err).
...
/// [`QuadStore`] so that:
///
/// - The LFTJ-specific surface area is named and documented as one cohesive
///   unit, rather than being interleaved with `QuadStore`'s core
///   CRUD/observability methods.
...
/// `impl LftjSource for MyStore {}` block. LOUDS and Ring engines override
/// these to enable the accelerated join path.
