...
/// Delete snapshot files strictly older than `keep_gen`, and WAL segment
/// files strictly older than `keep_seq`. Never touches a file whose numeric
/// suffix fails to parse (never delete something we don't understand), and
/// never touches the currently-committed generation/segment even if asked
/// (the `< keep_*` comparison already excludes them).
///
