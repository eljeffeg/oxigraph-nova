...
impl PyLiteral {
    fn from_value(value: &Bound<'_, PyAny>, datatype: Option<PyNamedNode>) -> PyResult<Self> {
        Ok(if let Some(datatype) = datatype {
            ...
            // XSD canonical double notation (e.g. `1.0E-1`), matching upstream
            // pyoxigraph's numeric-literal formatting. Neither `f64::to_string()`
            // nor `oxrdf`'s `From<f64>` impl (which just calls that) produce this
            // notation, so it is computed directly here via `format_xsd_double`.
            Literal::new_typed_literal(format_xsd_double(value), xsd::DOUBLE)
        } else {
...
/// Format a finite/non-finite `f64` as XSD canonical double notation
/// (e.g. `32100.0` -> `"3.21E4"`, `0.1` -> `"1.0E-1"`).
fn format_xsd_double(v: f64) -> String {
    ...
    // Rust's `{:e}` gives the shortest scientific notation, lowercase `e`.
    let s = format!("{v:e}");
    ...
    format!("{mantissa_canonical}E{exp_num}")
}
...
impl<'a, 'py> FromPyObject<'a, 'py> for OxStringInput {
    ...
    fn extract(obj: Borrowed<'a, 'py, PyAny>) -> PyResult<Self> {
        ...
        Ok(Self(decoded.to_string()))
    }
}

impl From<OxStringInput> for String {
    #[inline]
