...
pub fn ring_keep_heap() -> bool {
    match std::env::var("NOVA_RING_KEEP_HEAP") {
        Ok(v) => {
            let t = v.trim();
            !(t.is_empty() || t == "0" || t.eq_ignore_ascii_case("false") || t.eq_ignore_ascii_case("off"))
        }
        Err(_) => false,
    }
}
...
pub fn set_d1_tiny_merge_threshold_override(t: Option<u32>) {
    match t {
        ...
        Some(n) => D1_TINY_MERGE_OVERRIDE.store(u64::from(n.min(D1_TINY_MERGE_MAX)), Ordering::Relaxed),
    }
}
...
pub fn set_wedge_left_once_threshold_override(t: Option<u32>) {
    match t {
        ...
        Some(n) => {
            WEDGE_LEFT_ONCE_OVERRIDE
                .store(u64::from(n.min(WEDGE_LEFT_ONCE_MAX)), Ordering::Relaxed)
        }
    }
}
