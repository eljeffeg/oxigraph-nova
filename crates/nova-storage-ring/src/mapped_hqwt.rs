...
use crate::mapped_qwt::{
    AlignedBuf, MappedLevel, MappedQwtError, MAX_LEVELS, PAGE_ALIGN, align_up, level_get, level_rank,
    level_select,
};
...
impl<'m> MappedHqwtSection<'m> {
    ...
    fn n_occs_smaller_level(&self, level: usize) -> Result<[u64; 5], MappedQwtError> {
        ...
        let mut o = [0u64; 5];
        for i in 0..5 {
            o[i] = u64::from_le_bytes(self.sec[off_occs + i * 8..off_occs + i * 8 + 8].try_into().unwrap());
        }
        Ok(o)
    }
