...
//! `parse_body_triples` uses (see `crates/server/nova-server/src/lib.rs`), but keyed
...
//! `Vec` first: backends' `bulk_load` already accepts `impl
//! IntoIterator<Item = Quad>` and consumes it lazily, so for a single input
//! source (one file, or stdin) the parser's iterator is fed directly into
...
//! in parallel.
//!
//! ## Backend-agnostic bulk-load
//!
//! The streaming helpers take a `bulk_load` callback so both `LoudsStore`
//! and `RingStore` (and any future backend with the same bulk-load shape)
//! can share this module without hard-coding a store type.

use anyhow::{Context, Result};
use oxigraph_nova_core::{GraphName, Oxigraph, Quad};
...
/// Stream `files` (or stdin, if `files` is empty) into a store via a single
/// `bulk_load` call, returning the number of quads loaded.
///
/// `bulk_load` is backend-supplied (`LoudsStore::bulk_load`,
/// `RingStore::bulk_load`, …) so this helper stays storage-agnostic.
///
...
pub fn load_sources_into_store<F>(
    bulk_load: F,
    files: &[PathBuf],
    format: Option<&str>,
    graph: Option<&GraphName>,
...
where
    F: FnOnce(&mut dyn Iterator<Item = Quad>) -> Result<usize, Oxigraph>,
{
    match files {
        [] => load_stdin(bulk_load, format, graph, base, lenient),
        [single] => load_single_file(bulk_load, single, format, graph, base, lenient),
        multiple => load_multiple_files(bulk_load, multiple, format, graph, base, lenient),
    }
}
    ...
    F: FnOnce(&mut dyn Iterator<Item = Quad>) -> Result<usize, Oxigraph>,
{
    ...
    stream_into_store(
        bulk_load,
        make_parser(fmt, graph, base, lenient)?,
        reader,
        fmt,
    )
}

fn load_single_file<F>(
    bulk_load: F,
    path: &Path,
    format: Option<&str>,
    graph: Option<&GraphName>,
    ...
    F: FnOnce(&mut dyn Iterator<Item = Quad>) -> Result<usize, Oxigraph>,
{
    ...
    let mut quads = ErrorCapturingIter {
        inner: parser.for_reader(reader),
        error_slot: &error_slot,
    };

    let count = bulk_load(&mut quads).with_context(|| "bulk_load failed")?;
...
/// iterator suitable for bulk-load (which wants `Item = Quad`, not
/// `Item = Result<Quad, _>`): stashes the first error it encounters into
/// `error_slot` and then ends the iterator, so bulk-load stops cleanly at
/// the point parsing failed. The caller checks `error_slot` after bulk-load
/// returns to surface the failure.
