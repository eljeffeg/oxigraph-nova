...
pub enum Command {
    ...
    Load {
        ...
        location: PathBuf,
        /// Storage backend: `louds` (default, six-order LOUDS CompactLTJ) or
        /// `ring` (Cyclic-QWT RingStore).
        ///
        /// `--backend ring` requires building with `--features ring-backend`.
        /// Full-text is LOUDS-only.
        #[arg(long, default_value = "louds")]
        backend: String,
        /// File(s) to load
        ///
    ...
    /// `crates/server/nova-server/src/bin/nova_serve.rs`), exposed here as a
    ...
    Serve {
        ...
        ///
        /// `--backend ring` requires building with `--features ring-backend`.
        /// Both backends support in-memory and persistent `--location`
        /// (WAL + snapshot). Full-text is LOUDS-only.
        #[arg(long, default_value = "louds")]
        backend: String,
