//! Standalone Nova SPARQL server binary backed by a self-registered
//! [`StorageEngine`] (default LOUDS; Ring when built with `ring-backend`).
//!
//! Loads an RDF file into memory, builds the storage index (via
//! `compact()` / bulk_load), then serves the SPARQL 1.1 HTTP Protocol on the
//! given address. Used for external comparative benchmarking against Oxigraph
//! and QLever — see `benches/external/`.
//!
//! # Usage
//! ```bash
...
//!
//! Storage model note: without `--location`, the engine is always fully
...
//! being applied in memory — see each backend's WAL docs and `open` for
//! details of the overall persistent-storage design.
//!
//! Backend selection is via the self-registering
//! [`oxigraph_nova_core::BackendFactory`] registry. Product code never names
//! a concrete store type; deleting a storage crate simply removes that name
//! from `--backend` help / errors.
...
async fn main() {
    ...
    // Empty → default registered backend (louds when linked).
            ...
            "--help" | "-h" => {
                ...
                println!(
                    "Usage: nova_serve [--file <dataset> [--graph <iri>]]... [--location <dir>] \
                     ...
                     WAL-backed store rooted at <dir>. Short form: -l.\n\
                     Works with every registered --backend.\n\
                     --bind <addr> (matching `oxigraph serve --bind`), default 0.0.0.0:3030.\n\
                     Short form: -b.\n\
                     \n\
                     Without --location: purely in-memory store, --file is required.\n\
                     With --location <dir>: persistent WAL-backed store rooted at <dir>.\n\
                     If <dir> already has a WAL/snapshot, it is recovered and --file is\n\
                     ignored. If <dir> is empty/fresh and --file is given, the dataset is\n\
                     bulk-loaded and then persisted via a crash-safe snapshot commit.\n\
                     \n\
                     --backend <name>: storage backend (registered in this binary: {backends}).\n\
                     Default: louds when linked, else the first registered backend.\n\
                     `ring` requires building with `--features ring-backend`.\n\
                     \n\
                     ...
                     oxigraph_nova_core::SyncPolicy docs.\n\
                     \n\
    ...
    let backend = require_backend(&backend).unwrap_or_else(|e| panic!("{e}"));

    // ── Construct the store: persistent (--location) or in-memory ──────────
    let store = match &location {
        Some(dir) => {
            eprintln!(
                "[nova_serve] Opening persistent {backend} store at {} ...",
                dir.display()
            );
            let store = open_backend(backend, dir).unwrap_or_else(|e| panic!("open failed: {e}"));
            ...
            eprintln!(
                "[nova_serve] Recovered {} triples from WAL/snapshot.",
                store.triple_count()
            );
            store
        }
        None => {
            eprintln!("[nova_serve] Backend: {backend} (in-memory)");
            new_backend(backend).unwrap_or_else(|e| panic!("new_backend failed: {e}"))
        }
    };
    ...
    if !files.is_empty() {
        if store.triple_count() > 0 {
            ...
            // `bulk_load()` bypasses both the write buffer *and* the WAL
            // entirely: it builds the index directly in memory, then commits
            // via a single atomic snapshot + dictionary + WAL-segment-rotation +
            // MANIFEST commit. This matters enormously for `--location --file`:
            // the old path went through `extend()` → `insert()`, WAL-logging
            // and fsync-ing every single triple individually.
            let t0 = Instant::now();
            let count = store
    ...
    eprintln!(
        "[nova_serve] backend={backend} triple_count={}",
        store.triple_count()
    );

    // LOUDS-only memory diagnostics (best-effort; skipped for other backends).
    // Downcast is intentionally not used — product code stays type-erased.
    // Operators who need LOUDS-specific breakdowns can use dedicated tooling.
    ...
    let text_search: Option<Arc<dyn oxigraph_nova_core::TextSearch>> = if fulltext {
        ...
        store
            .enable_fulltext()
            .unwrap_or_else(|e| panic!("enable_fulltext failed: {e}"));
