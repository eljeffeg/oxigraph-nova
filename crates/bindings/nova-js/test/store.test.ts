...
describe("Store", () => {
    ...
    describe("#query()", () => {
        ...
        // NOTE: `use_default_graph_as_union`/`default_graph`/`named_graphs`
        // dataset-selection query options are not supported by this
        // Nova-backed store's `query()` (see `src/store.rs`'s `query()`,
        // which explicitly errors if any of them are passed) — the
        // corresponding upstream oxigraph-js sub-tests are intentionally
        // omitted here.
    ...
    describe("#load()", () => {
        ...
        it("load TriG with options", () => {
            const store = new Store();
            // `unchecked` is intentionally a no-op here, matching upstream
            // oxigraph-js exactly: upstream's own `src/store.rs::load()` only
            // ever reads a `LENIENT` reflect key (never an `UNCHECKED` one),
            // even though its TypeScript surface (`src/lib.rs`) and README
            // both still document an `unchecked?: boolean` load option. So
            // passing `unchecked: true` is silently ignored upstream too —
            // this isn't a Nova-specific gap, just faithfully vendored
