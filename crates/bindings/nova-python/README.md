...
cd crates/bindings/nova-python
...
`crates/bindings/nova-python/src/` (or any of its Nova dependencies) to pick up the
change.
...
`crates/server/nova-store/src/lib.rs`) rather than a reimplementation of
...
| `read_only()` | `Store` (class method) | No read-only storage mode in `nova-store`/`nova-engine-ring` |
| `remove_graph()` unregistration | `remove_graph()` | Implemented as `DROP GRAPH`, which Nova's SPARQL Update executor (`crates/core/nova-query/src/update.rs`) currently treats identically to `CLEAR GRAPH` — the graph quads are removed but the graph name isn't unregistered from `known_named_graphs()` |
...
storage-layer/hot-path change to `nova-core`/`nova-engine-ring`'s scan
...
reviewed as a `nova-core`/`nova-engine-ring` change, not a `nova-python`
one.

## Relationship to `crates/server/nova-store`


`PyStore` in `store.rs` holds a single `oxigraph_nova_store::Store` and
translates each pyoxigraph `Store` method 1:1 onto it — see
`crates/server/nova-store/src/lib.rs` for the facade this binding sits on top of,
