...
class TestStore(unittest.TestCase):
    ...
    @unittest.skip(
        "use_default_graph_as_union is not supported by this Nova-backed store's query()"
    )
    ...
    @unittest.skip("default_graph is not supported by this Nova-backed store's query()")
    ...
    @unittest.skip("named_graphs is not supported by this Nova-backed store's query()")
    ...
    @unittest.skip("custom_functions is not supported by this Nova-backed store's query()")
    ...
    @unittest.skip(
        "custom_aggregate_functions is not supported by this Nova-backed store's query()"
    )
    ...
    @unittest.skip("substitutions is not supported by this Nova-backed store's query()")
    ...
    @unittest.skip(
        "LOAD <url> requires the `http-client` feature, which this Nova-backed build "
        "is compiled without"
    )
    ...
    @unittest.skip(
        "remove_graph() is implemented as DROP GRAPH, which this Nova-backed store's "
        "SPARQL Update executor treats identically to CLEAR GRAPH (the graph name isn't "
        "unregistered from known_named_graphs()); see crates/core/nova-query/src/update.rs"
    )
    ...
    @unittest.skip(
        "Store.read_only() is not implemented by this Nova-backed store: there is no "
        "read-only storage mode in crates/server/nova-store/crates/nova-engine-ring"
    )
