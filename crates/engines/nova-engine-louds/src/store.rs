...
// Re-export so existing `oxigraph_nova_engine_louds::SyncPolicy` /
// `oxigraph_nova_engine_ring::SyncPolicy` imports keep compiling.
pub use oxigraph_nova_core::SyncPolicy;
use oxigraph_nova_core::{
    Dictionary, EmptyTrieIter, GRAPH_DEFAULT, GraphId, GraphName, LftjSource, NamedNode, Oxigraph,
    PhysicalShape, PreparedPhysicalOperator, Quad, QuadOp, QuadStore, StorageEngine, StoredQuad,
    Subject, Term, TermId,
};
#[cfg(feature = "fulltext")]
...
/// use oxigraph_nova_engine_louds::LoudsStore;
/// use std::sync::Arc;
///
...
impl StorageEngine for LoudsStore {
    fn engine_name(&self) -> &'static str {
        "louds"
    }

    fn compact(&self) -> Result<(), Oxigraph> {
        LoudsStore::compact(self)
    }

    fn backup(&self, destination: &Path) -> Result<(), Oxigraph> {
        LoudsStore::backup(self, destination)
    }

    fn set_sync_policy(&self, policy: SyncPolicy) {
        LoudsStore::set_sync_policy(self, policy)
    }

    fn set_auto_compact_threshold(&self, threshold: usize) {
        LoudsStore::set_auto_compact_threshold(self, threshold)
    }

    fn flush_wal(&self) -> Result<(), Oxigraph> {
        LoudsStore::flush_wal(self)
    }

    fn triple_count(&self) -> usize {
        LoudsStore::triple_count(self)
    }
    ...
    fn enable_fulltext(&self) -> Result<(), Oxigraph> {
        LoudsStore::enable_fulltext(self)
    }
...
fn louds_new_in_memory() -> Arc<dyn StorageEngine> {
    Arc::new(LoudsStore::new())
}

fn louds_open(path: &Path) -> Result<Arc<dyn StorageEngine>, Oxigraph> {
    Ok(Arc::new(LoudsStore::open(path)?))
}

inventory::submit! {
    oxigraph_nova_core::BackendFactory {
        name: "louds",
        description: "six-order LOUDS CompactLTJ (default production backend)",
        new_in_memory: louds_new_in_memory,
        open: louds_open,
    }
}
