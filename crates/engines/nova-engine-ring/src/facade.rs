...
use crate::{
    Col, CyclicRing, MappedRingA, MappedRingError, RowRange, open_novarng1_mmap,
...
impl BraidedRingIndex {
    ...
    /// Construct a mapped-only index from an already-open [`MappedRingA`].
    ///
    /// Used by `RingStore::open` to reattach a previously-materialised
    /// generation without rebuilding the heap QWT.
    pub fn from_mapped(mapped: MappedRingA) -> Self {
        Self {
            n: mapped.n,
...
mod tests {
    ...
    fn materialize_drops_heap_by_default() {
        ...
        idx.materialize_mapped_ex(&path, false)
            .expect("materialize");
        let _ = std::fs::remove_file(&path);
        assert!(idx.has_mapped());
        assert!(!idx.has_heap());
    ...
    fn d2_subject_range_intersection_matches_oracle_stream() {
        ...
        let objs = |s: u32| -> Vec<u32> { t.iter().filter(|x| x[0] == s).map(|x| x[2]).collect() };
